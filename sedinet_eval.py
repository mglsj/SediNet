...
def estimate_categorical(
    vars, csvfile, res_folder, dropout, numclass, greyscale, name, mode, wp
):
    """
    ...
    valid_gen = get_data_generator_1image(
        test_df,
        test_idx,
        True,
        ID_MAP,
...
###===================================================
def estimate_categorical_1image(
    vars, image, res_folder, dropout, numclass, greyscale, name, mode, wp
):
    """
...
def estimate_siso_simo(
    ...
    if scale == True:
        ...
        for var in vars:
            ...
            cs.fit_transform(np.r_[test_df[var].values].reshape(-1, 1))
            CS.append(cs)
            del cs
    else:
        CS = []
