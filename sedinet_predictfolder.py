...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "python sedinet_predictfolder.py -c configfile.json -i path/to/image_folder {-w weightsfile.hdf5} OR {-1 weightsfile_batch1.hdf5 -2 weightsfile_batch2.hdf5 -3 weightsfile_batch3.hdf5 -4 weightsfile_batch4.hdf5}"
        )
        sys.exit(2)
    for opt, arg in opts:
        if opt == "-h":
            print(
                "Example usage (single batch / weights file): python sedinet_predict1image.py \
            ...
            print(
                "Example usage (multiple batches / weights files): python sedinet_predict1image.py -c config/config_9percentiles.json \
    ...
    if len(auxin) > 0:
        auxin = config[
            auxin[0]
        ]  ##at least for now, just one 'auxilliary' (numerical/categorical) input in addition to imagery
        if len(vars) == 1:
            mode = "miso"
        elif len(vars) > 1:
            mode = "mimo"
    ...
    if mode == "siso" or mode == "simo":
        if numclass > 0:
            files = (
                glob(folder + os.sep + "*.jpg")
                + glob(folder + os.sep + "*.tif")
                + glob(folder + os.sep + "*.png")
            )
            result = []
            for image in files:
                result.append(
                    estimate_categorical_1image(
                        vars,
                        image,
                        res_folder,
                        dropout,
